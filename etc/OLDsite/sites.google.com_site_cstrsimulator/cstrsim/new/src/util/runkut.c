...
int procQuimRK(int argc, char *argv[]){
	...
	if(argc < 2){
		...
		fprintf(stdout, "File: config.txt\n");
	}else{
		conf = fopen(argv[1], "r");
		if(!conf){
			fprintf(stderr, "ERROR: could not open configuration file\n");
	...
	for(i=0;i<nf;i++)
		if( fl[i] != 0 )
	 		printf("Total # faults=%2d fault# =%2d extent=%7.2lf instant=%6.2f tau=%7.2lf\n",
	...
	while(simTime < maxTime) {
		...
		//Handles all faults
		initSensorFaults(h, q, t, c, cnt, ms, initVal);
		...
		if(h[1] <= 0.0){
			printf("\nEmergency shut down at iteration %f...\n", simTime);
			return 1;
		}
