...
C
      ...
      integer prot,dataout,fcnt,strlen,postrailblank
     ...
     1         R0(10),RRATE1,RRATE2,RC,RE,RV(18),SP(3),SDEV(18),sdn(14),
...
C Standard deviation of noise added to variable just after update

C nominal values
C      DATA sdn /0.001,0.001,0.001,0.001,0.001,0.001,0.001,0.001,0.001,
C     1                 0.001,0.001,0.001,0.001,0.001/

      DO I=1,14
        sdn(i) = 1E-4 / NORMVAL(I)
      END DO
...
C
C ADD NOISE
      call noise(CA0,1,DSEED,sdn)
      call noise(FLOW(1),2,DSEED,sdn)
      call noise(T(1),3,DSEED,sdn)
      call noise(T(3),10,DSEED,sdn)
      call noise(PCW,11,DSEED,sdn)
C
C
C
C
...
C
      ...
      call noise(FLOW(4),9,DSEED,sdn)
C
      FLOW(5)=(1D0/RC)*(PCW-JEP)**0.5D0
      call noise(FLOW(5),8,DSEED,sdn)
...
C
     ...
     1 (1/VOL)*(FLOW(1)*CA0-FLOW(2)*CA-RRATE1*VOLD-RRATE2*VOLD)*DT
      call noise(CA,5,DSEED,sdn)
C
      CB=(1/VOL)*(VOLD*CB)+
     1 (1/VOL)*(RRATE1*VOLD-FLOW(2)*CB)*DT
      call noise(CB,6,DSEED,sdn)
C
      CC=(1/VOL)*(VOLD*CC)+
...
C Add noise to a recently updated variable
      subroutine noise(var,I,DSEED,sdn)
      REAL*8 var,varold
      REAL*8 DSEED,sdn(14),RAND(1)
      INTEGER I

      varold = var
      CALL GGNML(DSEED,1,RAND)
      var=var+RAND(1)*sdn(I)
C      write(*,'(1x,A,I4,A,e30.20,A,e30.20)') 'NOISE: var #',
C     1    I,' ',varold,' --> ',var
      return
      end
C----------------------------------------------------------------------
