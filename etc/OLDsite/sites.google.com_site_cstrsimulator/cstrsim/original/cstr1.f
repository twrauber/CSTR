...
C
      ...
      integer prot, dataout, numhead,fcnt,strlen,postrailblank
      parameter(prot=17,dataout=18,numhead=6)
      logical isequal, labelchange
      character *40 classstr,auxstr,csvsep*1
      real *8 massbalold, epdold, cwpdold, molbalold, head(numhead),
     1        pumphead, deltahead, KL, sumConcABC, mol, dmolin,dmolout,
     1        qacc,qrxn,qextern,qc,qin,qjrleak,qout,myt2,
     1        mykA,mykB,mykC,mycA,mycB,mycC,aux,aux1,aux2



      REAL *8  ALPHA1,ALPHA2,BETA1,BETA2,B(3,3),CAO,CA,CB,CC,CCNOM,
...
C        write(prot,'(1x,A,A,I4,2(A,e12.5))') 'FAULT EXTENT DIFFENTIAL:',
C     1   ' S0=',S0,' EXTENT0(S0)=',EXTENT0(S0),' DEXT0(S0)=',DEXT0(S0)
      ENDIF
C
C CALCULATE FAULT EXTENT EXPONENTIAL GROWTH FUNCTION
C
      DEXT(S0)=EXP(TC(S0)*(DELAY(S0)-TIME))
C      write(prot,'(1x,A,A,I3,4(A,e12.5))')
C     1 'FAULT EXTENT EXPONENTIAL GROWTH FUNCTION:',
C     1 ' S0=',S0,' DELAY(S0)=',DELAY(S0),' TIME=',TIME,
C     1 ' TC(S0)=',TC(S0),' DEXT(S0)=',DEXT(S0)
C
...
C      if (.TRUE.) then
      if (.FALSE.) then
      write(prot,'(1x,A,3(A,I3),7(A,e12.5))') 'FAULT EXTENT:',
     1 ' fcnt=',fcnt,' M=',M,' S0=',S0,' DELAY(S0)=',DELAY(S0),
     1 ' TIME=',TIME,' TC(S0)=',TC(S0),' DEXT(S0)=',DEXT(S0),
     1 ' EXTENT0(S0)=',EXTENT0(S0),' DEXT0(S0)=',DEXT0(S0),
     1 ' VARIABLE=EXTENT0(S0)-DEXT0(S0)*DEXT(S0)=',
     1    EXTENT0(S0) - DEXT0(S0)*DEXT(S0)
      endif

C
C Produce class string at time t
C
C      write(*,*) 'fcnt=',fcnt,' classstr=',classstr
      if( fcnt.eq.1 .and. labelchange ) then
        labelchange = .false.
        if (F(1).le.10) then
          write(auxstr,'(I1)') F(1)
        else
...
C      write(*,'(1x,2(A,A))') 'auxstr=',auxstr,
C     1        'classstr=',classstr
      if( fcnt.eq.2 .and. labelchange ) then
        labelchange = .false.
        strlen=postrailblank(classstr)
        i=strlen
        classstr(i+1:i+1) = '_'
        if (F(2).le.10) then
          write(auxstr,'(I1)') F(2)
          classstr(i+2:i+2) = auxstr(1:1)
        else
C          write(auxstr,'(I2)') F(2)
          classstr(i+2:i+2) = auxstr(1:1)
          classstr(i+3:i+3) = auxstr(2:2)
        end if
C        classstr=trim(classstr)//'_'//auxstr
      end if
C      write(*,'(1x,2(A,A))') 'auxstr=',auxstr,'classstr=',classstr
      if( fcnt.eq.3 .and. labelchange ) then
        labelchange = .false.
        strlen=postrailblank(classstr)
        i=strlen
        classstr(i+1:i+1) = '_'
...
C        classstr=trim(classstr)//'_'//auxstr          !No Fortran 77
      end if
C
60    CONTINUE
C
C      write(*,'(1x,2(A,I3),2(A,A))') 'M=',M,' S0=',S0,' auxstr=',
C     1         auxstr,'classstr=',classstr
C
100   CONTINUE
C
...
C
      FLOW(2)=(1D0/RE)*(PP+PB-REP)**0.5D0
      FLOW(3)=(1D0/R(2))*((PP+PB-REP)-(FLOW(2)*R(1))**2.0D0)**0.5D0
      FLOW(4)=FLOW(2)-FLOW(3)
C
      FLOW(5)=(1D0/RC)*(PCW-JEP)**0.5D0
      FLOW(6)=(1D0/R(7))*(PCW-JEP-(FLOW(5)*(R(5)+R(6)))**2.0D0)**0.5D0
      FLOW(7)=(1D0/R(8))*(PCW-JEP-(FLOW(5)*(R(5)+R(6)))**2.0D0)**0.5D0
      FLOW(8)=FLOW(5)-FLOW(6)-FLOW(7)
C      if (.TRUE.) then
      if (.FALSE.) then
      write(prot,'(1x,A,8(A,e12.5),A,3(f7.2),2(A,f6.2,A))')
     1    'FLOWRATES Q=',
...
C      if (.TRUE.) then
     ...
     1    ' RHO1=',RHO1, ' RHO2=',RHO2, ' CP=',CP, ' VOL=',VOL
      ...
      ENDIF
      PB=RHO*LEVEL
      head(1) = LEVEL ! After the reactor outlet, before pump

      pumphead = -PP/RHO
C     write(prot,'(1x,2(A,e12.3))') 'Level=',LEVEL,' pumphead=',pumphead
      deltahead = pumphead
      head(2) = head(1) - deltahead ! After pump

      deltahead = R(1)*R(1)*FLOW(2)*FLOW(2)/RHO
      KL = deltahead/(FLOW(2)*FLOW(2))
      head(3) = head(2) - deltahead ! After pipe between pump and branch leak-outlet
C     write(prot,'(1x,4(A,e12.5))') 'Q2=',FLOW(2),' R1=',R(1),
C    1 ' dh23(pipe)=',deltahead,' KL(pipe)=',KL

      deltahead = R(2)*R(2)*FLOW(3)*FLOW(3)/RHO
      KL = deltahead/(FLOW(3)*FLOW(3))
      head(6) = head(3) - deltahead ! After leak
C     write(prot,'(1x,5(A,e12.5))') 'Q3=',FLOW(3),' R2=',R(2),
C    1 ' dh36(leak)=',deltahead,' KL(leak)=',KL,' m=',1.0D0/SQRT(KL)

      deltahead = R(3)*R(3)*FLOW(4)*FLOW(4)/RHO     
      head(4) = head(3) - deltahead ! After valve
      KL = deltahead/(FLOW(4)*FLOW(4))
C     write(prot,'(1x,5(A,e12.5))') 'Q4=',FLOW(4),' R3=',R(3),
C    1 ' dh34(valve)=',deltahead,' KL(valve)=',KL,' m=',1.0D0/SQRT(KL)

      deltahead = R(4)*R(4)*FLOW(4)*FLOW(4)/RHO
      KL = deltahead/(FLOW(4)*FLOW(4))
      head(5) = head(3) - deltahead ! After outlet
C     write(prot,'(1x,4(A,e12.5))') 'Q4=',FLOW(4),' R4=',R(4),
C    1 ' dh45(outlet pipe)=',deltahead,' KL(outlet pipe)=',KL

C     write(prot,'(1x,A,6(e12.3))') 'Heads:',(head(I),I=1,6)
C
C
...
C
C Verify a simpler version for concentrations
C      mykB=ALPHA1*EXP(-BETA1/(8.314D0*(273.15D0+T(2))))
C      mykC=ALPHA2*EXP(-BETA2/(8.314D0*(273.15D0+T(2))))

      mycA=mycA+
     1 DT*((-RRATE1-RRATE2)+(1/VOLD)*(FLOW(1)*(CAO-mycA)-FLOW(6)*mycA)) 
      mycB=mycB+DT*(RRATE1-(mycB/VOLD)*(FLOW(1)+FLOW(6)))
      mycC=mycC+DT*(RRATE2-(myCC/VOLD)*(FLOW(1)+FLOW(6)))
C
C
      CA=(1/VOL)*(VOLD*CA)+
...
C      if (.TRUE.) then
      if (.FALSE.) then
      write(prot,'(1x,A,A,L2,A,e9.2,15(A,e11.4))')
     1 'Rates,concentrations,Temps:',
     1  'eq=',isequal,' diffC=',mycC-CC,' err rel=',100*(mycC-CC)/CC,
     1  ' r1=',RRATE1,' r2=',RRATE2,
     1  '  cA=',CA,' mycA=',mycA,' cB=',CB,' mycB=',mycB,' cC=',CC,
     1  ' mycC=',mycC,' sumC=',CA+CB+CC,' mysumC=',mycA+mycB+mycC,
     1  '  T1=',T(1),' T2=',T(2),' T3=',T(3),' T4=',T(4)

      write(prot,'(1x,A,8(A,e12.5))') 'Flows:',
     1  'Q1=',FLOW(1),' Q2=',FLOW(2),' Q3=',FLOW(3),' Q4=',FLOW(4),
     1  ' Q5=',FLOW(5),' Q6=',FLOW(6),' Q7=',FLOW(7),' Q8=',FLOW(8)
      endif
     
     
      qin=FLOW(1)*RHO1*CP1*T(1)
      qout=FLOW(2)*RHOLD*CPOLD*T(2)
      qrxn=(QREAC1*RRATE1+QREAC2*RRATE2)*VOLD
      qc=UA*(T(2)-T(4))
      qjrleak=FLOW(6)*RHO2*CP2*T(4)
      qextern=QEXT
      qacc=qin-qout+qrxn-qc+qjrleak+qextern
      
      
      myt2=myt2 + DT*((qrxn+qextern-qc)/(RHO*CP) + 
     1  FLOW(1)*(T(1)-myt2) + FLOW(6)*(T(4)-myt2))
      
C      if (.TRUE.) then
      if (.FALSE.) then
      write(prot,'(1x,A,10(A,e12.5),A,f9.5,A)') 'Heat flows dq/dt:',
     1  ' in=',qin,' rout=',qout,' rxn=',qrxn,
     1  ' c=',qc,' jrleak=',qjrleak,' ext=',qextern,' acc=',qacc,
     1  ' T(2)=',T(2),' myt2=',myt2,' T(2)-myt2=',T(2)-myt2,
     1  ' err rel=',100*(T(2)-myt2)/T(2),'%'
      endif
C
...
C
      MEAS(1,2)=CAO
      MEAS(2,2)=FLOW(1)
C      write(prot,'(1x,A,e12.5)') 'Feed flowrate=', FLOW(1)
      MEAS(3,2)=T(1)
      MEAS(4,2)=LEVEL
      MEAS(5,2)=CA
      MEAS(6,2)=CB
...
C      if (.TRUE.) then
      if (.FALSE.) then
      write(prot,'(1x,A,e12.5,A,e12.5,A,e12.5,A,e8.3,A,e8.3,
     +  A,e12.5,A,e12.5,A,e12.5)')
     +  'Inventory constraint: FINT=',FINTEG,' Fin=',MEAS(2,2),
     +  ' Fout=',MEAS(9,2),' DT=',DT,' TAREA=',TAREA,
     +  ' L=',MEAS(4,2),' R0 old=',massbalold,' R0=',MASSBAL
      endif
C
...
C      if (.TRUE.) then
      if (.FALSE.) then
      write(prot,'(1x,A,e9.2,A,e14.5,A,e14.5,A,e14.5,A,e14.5,
     +  A,e12.5,A,e12.5)')
     +  'Effluent flow constraint: RHO1=',RHO1,' L=',MEAS(2,2),
     +  ' Delta Pp0=',PP0,' CL=', 100.0D0-MEAS(12,2),
     +  ' Reffluent=',R0(1),'+',R0(4),'=',R0(1)+R0(4)
      endif
C
...
C      if (.TRUE.) then
      if (.FALSE.) then
      write(prot,'(1x,A,e12.5,A,e12.5,A,e12.5,A,e12.3,A,e8.3,
     +  10(A,e11.4))')
     +  'Mol bal. constr.: MolBal=',MOLBAL,' MolIn=',MOLIN,
     +  ' MolOut=',MOLOUT,' cA+cB+cC=',sumConcABC,' cA=',MEAS(5,2),
     +  ' cB=',MEAS(6,2),' cC=',CC,' VOL=',VOL,
     +  ' F1-F4=',FLOW(1)-FLOW(4),' F2=',FLOW(2),
     +  ' F3=',FLOW(3),' F6=',FLOW(6),
     +  ' dMolDiffInOut=',dmolin-dmolout,' MolDiffInOut=',MOLIN-MOLOUT,
     +  ' dmol=',mol-60.0D0
      endif
C
...
270   CONTINUE
      write(dataout,'(18(e12.5,A1),A1,A10)')
