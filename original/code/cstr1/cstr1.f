...
C
      ...
      integer prot,dataout,numhead,fcnt,strlen,postrailblank,posclassstr
...
C
     ...
     1            'ABNORMAL JACKET EFFLUENT PRESSURE       ', ! Fault 17, =JEP, nominal value is 0.0
...
37    CONTINUE
      ...
      IF (F(S0) .EQ. 17) WRITE (*,835) JEP  ! Fault 17, =JEP, nominal value is 0.0
...
C      if (.TRUE.) then
      if (.FALSE.) then
      write(prot,'(1x,A,9(A,e10.3),A,3(f7.2),2(A,f6.2,A))')
     1    'FLOWRATES Q:',
...
C      mykC=ALPHA2*EXP(-BETA2/(8.314D0*(273.15D0+T(2))))
      ...
      mycA=mycA+
     1 DT*((-RRATE1-RRATE2)+(1/VOLD)*(FLOW(1)*(CA0-mycA)-FLOW(6)*mycA)) 
...
C      if (.TRUE.) then
      if (.FALSE.) then
      write(prot,'(1x,A,A,L2,A,e9.2,16(A,e11.4))')
     1 'Rates,concentrations,Temps:',
     1  'eq=',isequal,' diffC=',mycC-CC,' err rel=',100*(mycC-CC)/CC,
     1  ' r1=',RRATE1,' r2=',RRATE2,
     1  '  cA0=',CA0,'  cA=',CA,' mycA=',mycA,' cB=',CB,' mycB=',mycB,
     1  ' cC=',CC,' mycC=',mycC,' sumC=',CA+CB+CC,' mysumC=',
     1  mycA+mycB+mycC,'  T1=',T(1),' T2=',T(2),' T3=',T(3),' T4=',T(4)
...
C        DO 250 S0=1,M ! https://stackoverflow.com/questions/63730527/outdated-do-loops-in-fortran-2018
        DO S0=1,M
...
C      if (.FALSE.) then
      write(*,
C      write(prot,
     + '(1x,A,e12.5,A,e12.5,A,e12.5,A,e12.3,A,e8.3,
     +  11(A,e11.4))')
     +  'Mol bal. constr.: MolBal=',MOLBAL,' MolIn=',MOLIN,
     +  ' MolOut=',MOLOUT,' cA0=',MEAS(1,2),' cA+cB+cC=',sumConcABC,
     +  ' cA=',MEAS(5,2),' cB=',MEAS(6,2),' cC=',CC,' VOL=',VOL,
