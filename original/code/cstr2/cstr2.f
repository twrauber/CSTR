...
C
C Normal CNT2 41.7 is wrong, since in first simlation loop changes to 40.7
...
C
     ...
     1                           0.0D0,0.0D0,0.0D0/
C
C      data LEVEL /0.2D0/  ! Original value is wrong, since in first simlation loop is 2m
...
30    CONTINUE
C
35    S0=S0 + 1         ! counter of the fault (one or more)
...
C
     1  EXTENT0(S0),DELAY(S0)
      ...
      ENDIF
      write(*,*) 'F(S0)=',F(S0),'S1(S0)=',S1(S0),'S2(S0)=',S2(S0)
47    CONTINUE
...
C      if(.TRUE.) THEN
      if(.FALSE.) THEN
        ...
        CALL MEASOUT( dataout, MEAS, csvsep, classstr, iter )
        write(*, '(A11,18(A15,A1))')
     1         '',(SENSORACRONYMS(I),csvsep,I=1,18)
        CALL PEEPVARS(MEAS,classstr,iter)
C      stop
      END IF
...
C
      ...
      if( S0 .eq. 2 ) then
        if( fcnt .gt. 1) then
          posclassstr=posclassstr+1
          classstr(posclassstr:posclassstr) = '+'
        endif
        if (F(S0).le.10) then
          write(auxstr,'(I1)') F(S0)
          posclassstr=posclassstr+1
...
60    CONTINUE
C
C      write(*,*) 'ITERATION=',iter,'TIME=',TIME,'M=',M,' S0=',S0,
C     1   ' F(1)=',F(1),' F(2)=',F(2),
...
C
      ...
      dmolout=sumConcABC*MEAS(9,2)*DT ! (CA + CB + CC Nominal) * FLOW(4) * DT
      MOLIN=MOLIN+dmolin
...
C ZCOUNT                - COUNTER FOR PRINTING OUTPUT
     ...
     1    '  Time Horizon=',TH,'  Delta t=',DT
        write(*,*) 'Terminating at iteration =', iter, 'TIME=',TIME
...
C Dump the measurements to the output file
      SUBROUTINE MEASOUT( dataout, MEAS, csvsep, classstr, iter )
      integer dataout, iter
      REAL*8 MEAS(18,2)
      character*1 csvsep
...
C Peep measured variables
      SUBROUTINE PEEPVARS(MEAS,classstr,iter)
...
C      WRITE(*,*) 'Peeping variables ...'
     ...
     1               'k=',iter,(MEAS(I,2),csvsep,I=1,18),' ',classstr
C      write(*, *) '...'
C      read(*,*) dummy
      RETURN
      END
